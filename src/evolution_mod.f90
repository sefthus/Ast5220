...
contains
  ...
  subroutine initialize_perturbation_eqns
    ...
    do i = 1, n_k
       ...
       Psi(0,i) = - Phi(0,i) - 12.d0*H_0**2.d0/(c*ks(i)*a_init)**2.d0*Omega_r*Theta(0,2,i)
    end do
  ...
  subroutine integrate_perturbation_eqns
    ...
    do k = 1, n_k
       ...
       do while(x_t(i_tc)<= x_tc)
          ...
          Psi(i_tc,k)        = - Phi(i_tc,k) - 12.d0*H_0**2.d0/(ck*a_t)**2.d0*Omega_r*Theta(i_tc,2,k)
       ...
       do i = i_tc, n_t
          ...
          dPsi(i,k)     = -dPhi(i,k) - 12.d0*H_0**2.d0/(ck*exp(x_t(i)))**2 * Omega_r*(dTheta(i,2,k)-2.d0*Theta(i,2,k))
       end do
  ...
  subroutine dy_tc_dx(x, y, dydx)
    ...
    ! Derivatives
    Theta2  = - 20.d0/45.d0 *ckH_p/dt * Theta1
    R       = 0.75d0*Omega_r/(Omega_b*a)
    Psi     = - Phi - 12.d0*H_0**2.d0/(ck*a)**2.d0 * Omega_r * Theta2
    dPhi    = Psi - ckH_p**2.d0/3.d0*Phi + 0.5d0*(H_0/H_p)**2.d0 * (Omega_m/a*delta + Omega_b/a*delta_b + 4.d0*Omega_r/a**2.d0*Theta0)
    dv = - v - ckH_p * Psi
    ...
    q    = (-((1.d0-2.d0*R)*dt + (1.d0+R)*ddt)*(3.d0*Theta1 + v_b)- ckH_p*Psi + (1-(dH_p/H_p))*ckH_p*(-Theta0 + 2.d0*Theta2) - ckH_p*dTheta0)/((1.d0+R)*dt + (dH_p/H_p) -1.d0) 
  ...
  subroutine dy_dx(x, y, dydx)
    ...
    do l = 2, lmax_int-1
        dydx(6+l) = l*ckH_p/(2.d0*l+1.d0)*y(6+l-1) - (l+1.d0)/(2.d0*l + 1.d0)*ckH_p*y(6+l+1) + dt*(y(6+l) - 0.1d0*y(6+l)*abs(l==2))
    end do
